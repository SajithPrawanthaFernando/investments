...
export default function PropertyDetail({
  ...
  // Fallback to dummy images if your data doesn't have a gallery array yet
  const galleryImages = (property as any).gallery || [
    property?.image,
  ...
  useGSAP(
    () => {
      // Triggers a smooth fade and slight scale-up every time the active image changes
      gsap.fromTo(
                ...
                {galleryImages.map((img: string, index: number) => (
                  <button
                    ...
                    className={cn(
                      ...
                      activeImage === img
                        ? "border-brand opacity-100" // Active state: Blue border, full opacity
                        : "border-transparent opacity-60 hover:opacity-100", // Inactive state
                    )}
                  >
                    <div
